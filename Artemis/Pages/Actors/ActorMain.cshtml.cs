...
{
    public class ActorMainModel : BasePageModel
    {
        ...
        public ActorMainModel(ArtemisDbContext context, IActorService actorService) : base(context)
        {
            db = context;
        ...
        public JsonResult OnPostCreate([DataSourceRequest] DataSourceRequest request, [Bind(Prefix = "models")] IEnumerable<ActorViewModel> data)
        {
            if (data != null && ModelState.IsValid)
            {
                ...
                {
                    if (!actorService.Exists(item.FullName))
                    {
                        actorService.Create(item);
                    }
                    else
                    {
                        ModelState.AddModelError(string.Empty, "Actor with this name already exists. Operation cancelled.");
                    }
                }
            }
